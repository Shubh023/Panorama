...
void getClicks(Window w1, Window w2,
    ...
    // Polling all clicks until user presses the Right Button
    while (anyGetMouse(p, win, sub_win) != 3) {
        setActiveWindow(win);
        // Handle Click on Window W1
        if(win == w1){
            fillCircle(p, 2.5, RED);
            cout << "Clicked Window W1 at : (" << p << ")" << endl;
            pts1.push_back(p); 
        }
        // Handle Click on Window W2
        else if (win == w2) {
            fillCircle(p, 2.5, GREEN);
            cout << "Clicked Window W2 at : (" << p << ")" << endl;
            pts2.push_back(p);
        };
...
Matrix<float> getHomography(const vector<IntPoint2>& pts1,
    ...
    // Solving Equation for Ai * h = 0 with i being the 
    for(size_t i = 0; i < n; i++){
        // Handling Matrix A and we only take in account the first two lines of Ai
        A(2 * i, 0) = pts1[i].x();
        A(2 * i, 1) = pts1[i].y();
        A(2 * i, 2) = 1;
        A(2 * i, 3) = 0;
...
void panorama(const Image<Color,2>& I1, const Image<Color,2>& I2,
    ...
    I.fill(WHITE);
    // ------------- TODO/A completer ----------
    // We take final image I and add all the pixels information from I1 & I2 to create the panorama from them
    for (int i = 0; i < I.width(); i++) {
        for (int j = 0; j < I.height(); j++) {
            // We will store information from x0 and y0 into v temporarily
            v[0] = i + x0;
            v[1] = j + y0;

            /**
             *  We will fill the parts of I that can be filled with I2 
             *  before adding the pixels information from I in the next step
             */
            if(v[0] > 0 && v[1] > 0 && v[0] < I2.width() && v[1] < I2.height())
                I(i,j) = I2(v[0],v[1]);

            /**
             * We are using the method where we try to pull pixels from original image by transformation
             * therefore we will calculate the inverse of H and apply it on v to get the pixel information we need
             */
            v = inverse(H) * v;
            v /= v[2];

            // Verifying if there is any overlapping and calculating the pixel information to use accordingly
    ...
    // Save Image to check the resulting panorama
    save(I,"Result.jpg",100);
